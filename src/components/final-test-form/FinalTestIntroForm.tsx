...
import IntroPostTestQuestoins from "@/data/final-test/IntroPostTestQuestions.json";

interface UserAnswers {
  [questionId: number]: string;
}

const FinalTestIntroForm = () => {
  ...
  const confirmationHandler = () => {
    ...
    for (let i = 0; i < IntroPostTestQuestoins.length; i++) {
      if (userAnswers[i + 1] === IntroPostTestQuestoins[i].answer) {
        setScore((totalScore += IntroPostTestQuestoins[i].score));
      }
    }
  ...
  return (
    <>
      ...
      <div className="dashboard-content-container mb-4">
        ...
        {IntroPostTestQuestoins.map((question) => (
...
export default FinalTestIntroForm;
